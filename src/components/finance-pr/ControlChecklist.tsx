...
export function ControlChecklist({ report, activeRevIndex }: ControlChecklistProps) {
  ...
  const familyLabels: Record<string, { title: string; icon: React.ElementType }> = {
    evidence_lineage: { title: 'Supporting documents', icon: ShieldCheck },
    accounting_integrity: { title: 'Accounts & amounts', icon: ShieldCheck },
    policy_provenance: { title: 'Company policy & FX', icon: ShieldAlert },
  };
    ...
    <SpotlightCard className="overflow-hidden p-0">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-black/[0.06] bg-zinc-50/80 px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-sm font-medium text-zinc-800">Automated checks</h4>
        </div>
        {report.blocked ? (
