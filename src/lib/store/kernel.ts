 ...
 *
 * Append-only events, immutable proposal revisions, sandbox ledger posting,
 * and reconciliation close. Backed by the frozen benchmark in bench/fixtures/.
 */

type Fixture = BenchmarkFixture;
...
function bindDataAccess(): void {
  bindRuntime({
    bankLines: () => state().fixture.bankLines,
    ledgerEntries: () => state().fixture.ledgerEntries,
    documents: () => state().fixture.documents,
    fxObservations: () => state().fixture.fxObservations,
    controlPRs: () => state().controlPRs,
...
/* ------------------------------------------- record reads (agent tools) */

export { getBankLine, getSupportingDocument, listBankLines, listFxObservations, listLedgerEntries, listSupportingDocuments } from '@/lib/data/access';
